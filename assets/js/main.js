...
function getBingImages(imgUrls) {//背景
	/**
	 * 获取Bing壁纸
	...
	getBackImage();//首次加载背景图
	setInterval(getBackImage,60000);//每隔1分钟更换背景
}

document.addEventListener('DOMContentLoaded', function () {//名言
	function getContent(flag=false){//获取一言数据
		if (flag == true){//使用一言数据
			...
			xhr.onreadystatechange = function () {
				if (this.readyState == 4 && this.status == 200) {
					var reses = JSON.parse(this.responseText);
					var randomNumber = Math.floor(Math.random() * (reses.length));
					res = reses[randomNumber]
					document.getElementById('description').innerHTML = res.hitokoto + "<br/> -「<strong>" + res.from + "</strong>」";
				}
			};
			xhr.open("GET", "assets/json/sayings.json", true);
			xhr.send(null);
		}
	}
	
	function showContent(){//展示数据
		getContent();
	...
	showContent();//首次加载内容
	setInterval(showContent,60000);//每隔1分钟更换内容
});
